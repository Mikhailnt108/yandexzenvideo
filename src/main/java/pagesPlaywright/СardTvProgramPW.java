...
public class СardTvProgramPW extends BasePagePlaywright {
    ...
    public void checkAutoStartVideoPlayer(String login, String password) throws InterruptedException {
//        BrowserContext context = browserHeadfull.newContext(new Browser.NewContextOptions().setStorageStatePath(Paths.get("state.json")));
//        pageHeadfull = context.newPage();
        pageHeadfull.navigate("https://web-preprod5.megafon.tv/tv/tv_catalog");
        ...
        pageHeadfull.waitForSelector("//div[text()='Введите номер телефона']");
        pageHeadfull.fill("//input[@name='login']", login);
        pageHeadfull.click("//button[text()='Далее']");
        pageHeadfull.fill("//input[@type='password']",password);
        pageHeadfull.click("//button[text()='Войти']");
        pageHeadfull.waitForSelector("(//span[contains(text(),'+792')])[2]");
        pageHeadfull.reload();
        pageHeadfull.waitForLoadState();
        if (pageHeadfull.querySelectorAll("//div[@aria-label='Notification']").size() != 0) {
        ...
        if(pageHeadfull.querySelectorAll("//button[contains(@class,'M2wxcFvZLf83aNlb6Ab1V _1FfeR84AXAbi63sErW3rma')]").size()!=0) {
            pageHeadfull.click("//button[contains(@class,'M2wxcFvZLf83aNlb6Ab1V _1FfeR84AXAbi63sErW3rma')]");
            pageHeadfull.waitForSelector("//button[text()='Подтвердить']");
            pageHeadfull.click("//button[text()='Подтвердить']");
    ...
    public void checkImagePlayerСardTvProgramUnrecorded() throws IOException, InterruptedException {
        ...
        pageHeadfull.evaluate("n => n.innerText='Название канала'", nameTvChannelCrumbs);

        ElementHandle nameTvProgramCrumbs = pageHeadfull.querySelector("(//span[@itemprop='name'])[4]");
        pageHeadfull.evaluate("n => n.innerText='Название передачи'", nameTvProgramCrumbs);

        ElementHandle imgTvChannel = pageHeadfull.querySelector("._3lvyrUkaGN_QANJMIvjX5c");
        pageHeadfull.evaluate("i => i.setAttribute('src', 'https://static-sesure.cdn.megafon.tv/images/Channel/7f/4e/62d9c9b6d708dcaf23d41a28c8b9b4b699a1/logo_tv_guide__web-wp.png')", imgTvChannel);

        ElementHandle nameTvChannel = pageHeadfull.querySelector("//h1[@class='_1v_D6wOANknQeJMBPo_rKK']");
        pageHeadfull.evaluate("nC => nC.innerText='Название канала'", nameTvChannel);

        ElementHandle genres = pageHeadfull.querySelector("(//div[@class='_364E2xRe8IGMOTfCluwbl2'])[1]");
        pageHeadfull.evaluate("d => d.innerText='Сегодня, 00:00, 18+'", genres);

        ElementHandle dateTimeAge = pageHeadfull.querySelector("(//div[@class='_364E2xRe8IGMOTfCluwbl2'])[2]");
        pageHeadfull.evaluate("g => g.innerText='Познавательные, HD, В записи'", dateTimeAge);

        ElementHandle nameTvProgram = pageHeadfull.querySelector("//a[@class='_1nAXLMkHN0PXnwvulfBvK0']");
        pageHeadfull.evaluate("nP => nP.innerText='Название передачи'", nameTvProgram);

        ElementHandle videoTvProgram = pageHeadfull.querySelector("//video[@src]");
        pageHeadfull.evaluate("vP => vP.setAttribute('src', 'blob:notVideo')", videoTvProgram);

        ElementHandle descriptionTvProgram = pageHeadfull.querySelector("//div[@class='_3DKCWpmCv7Gf2hQA0mQQ2g']");
        pageHeadfull.evaluate("dP => dP.innerText='С помощью последних научных данных и потрясающей трёхмерной " +
                "компьютерной графики вы проникнете внутрь звёзд, планет и галактик и исследуете тайны " +
                "происхождения и будущего Вселенной.'", descriptionTvProgram);
        ...
        pageHeadfull.evaluate("dP => dP.innerText='Канал находится на острие научного и технологического поиска, рассказывает об изобретениях, " +
                "показывая, как наука воздействует на нашу жизнь. Охватывается весь спектр научных интересов – от изучения нашей Вселенной " +
                "до практической науки вокруг нас.'", descriptionTvChannel);

        ElementHandle userLogin = pageHeadfull.querySelector("(//span[@class='ch-trigger__title ch-trigger__title_view_lk'])[2]");
        pageHeadfull.evaluate("uL => uL.innerText='+79260010101'", userLogin);

        pageHeadfull.querySelector("//div[@class='_3oIAMUjIv-QAdeSq_k6cql']").hover();

        ElementHandle nameProgramInPlayer = pageHeadfull.querySelector("//div[@class='_2GPoEznIkBV65Iqkud1teP']");
        pageHeadfull.evaluate("nP => nP.innerText='Название передачи'", nameProgramInPlayer);
        ...
        pageHeadfull.evaluate("nNC => nNC.innerText='01 - Название канала'", numberNameTvChannelInPlayer);

        List<ElementHandle> progressVideoAll;
        for (int i = 0; i < pageHeadfull.querySelectorAll("//div[@class='_2wsl4lGkd8OHfFTRCpObeb _1EUAQDMdNFPAPHIXjrbxxi']").size(); i++) {
            progressVideoAll = pageHeadfull.querySelectorAll("//div[@class='_2wsl4lGkd8OHfFTRCpObeb _1EUAQDMdNFPAPHIXjrbxxi']");
            pageHeadfull.evaluate("pV => pV.innerText='0'", progressVideoAll.get(i));
        }
        ...
        for (int i = 0; i < pageHeadfull.querySelectorAll("//div[contains(@class,'oog6D4ljOTvbih-_NivDx')]").size(); i++) {
            posterOtherTvBackgroundAll = pageHeadfull.querySelectorAll("//div[contains(@class,'oog6D4ljOTvbih-_NivDx')]");
            pageHeadfull.evaluate("pB => pB.setAttribute('style', 'background-color: rgb(89, 156, 183); background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(https://static-sesure.cdn.megafon.tv/images/img/52/05/38adc5d8194094c15f274d89761e4d5231a6/tile_player__web-wp.webp);')", posterOtherTvBackgroundAll.get(i));
        ...
        List<ElementHandle> nameTvProgramAll;
        List<ElementHandle> ageAll;
        for (int i = 0; i < pageHeadfull.querySelectorAll("//div[@class='_2F06JEFCaBoh-jQ-JAfmAN']").size(); i++) {
            timeAll = pageHeadfull.querySelectorAll("//div[@class='_2F06JEFCaBoh-jQ-JAfmAN']");
            pageHeadfull.evaluate("t => t.innerText='00:00'", timeAll.get(i));
            nameTvProgramAll = pageHeadfull.querySelectorAll("//div[@class='DSoUWlplI4m8T5WSf-XxC']");
            pageHeadfull.evaluate("nP => nP.textContent='Название передачи'", nameTvProgramAll.get(i));
            ageAll = pageHeadfull.querySelectorAll("//div[contains(@class,'_3RTKiE8VDgo764HGa4WvpJ _3uK4RWVSuUFLQ2ZmeFzsQi')]");
        ...
        pageHeadfull.evaluate("dA => dA.innerText='Вс, 1'", dayAgo);

        ElementHandle afterTomorrow = pageHeadfull.querySelector("(//div[@class='_29P6Zrf7K1Dx_N1l0bKL45'])[last()]");
        pageHeadfull.evaluate("aT => aT.innerText='Чт, 4'", afterTomorrow);
    ...
    public void checkImageScheduleСardTvProgramUnrecorded() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < pageHeadfull.querySelectorAll("//div[@class='_2F06JEFCaBoh-jQ-JAfmAN']").size(); i++) {
            timeInPlayerAll = pageHeadfull.querySelectorAll("//div[@class='_2F06JEFCaBoh-jQ-JAfmAN']");
            pageHeadfull.evaluate("t => t.innerText='00:00'", timeInPlayerAll.get(i));
            nameTvProgramInPlayerAll = pageHeadfull.querySelectorAll("//div[@class='DSoUWlplI4m8T5WSf-XxC']");
            pageHeadfull.evaluate("nP => nP.textContent='Название передачи'", nameTvProgramInPlayerAll.get(i));
            ageInPlayerAll = pageHeadfull.querySelectorAll("//div[contains(@class,'_3RTKiE8VDgo764HGa4WvpJ _3uK4RWVSuUFLQ2ZmeFzsQi')]");
            pageHeadfull.evaluate("a => a.innerText='18+'", ageInPlayerAll.get(i));
        }
        List<ElementHandle> timeAll;
        List<ElementHandle> nameTvProgramAll;
        List<ElementHandle> ageAll;
        ...
        pageHeadfull.evaluate("dA => dA.innerText='Вс, 1'", dayAgoInPlayer);
        ElementHandle afterTomorrowInPlayer = pageHeadfull.querySelector("(//div[@class='_29P6Zrf7K1Dx_N1l0bKL45'])[5]");
        pageHeadfull.evaluate("aT => aT.innerText='Чт, 4'", afterTomorrowInPlayer);
...
//        page.querySelector("//div[@class='_3oIAMUjIv-QAdeSq_k6cql']").hover();
    ...
    public void checkImageScheduleСardTvProgramRecorded() throws IOException, InterruptedException {
        page.click("(//button[@class='_1y2MwvAuO97Xb0-8ccbmkk'])[2]");
