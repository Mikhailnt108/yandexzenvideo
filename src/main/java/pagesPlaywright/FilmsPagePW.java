...
public class FilmsPagePW extends BasePagePlaywright {
    ...
    public void checkImageFilmsPageScrollForUnauthorized() throws IOException, InterruptedException {
        ...
        pagePlayer.navigate("https://web-preprod5.megafon.tv/movies/vods");
        ...
        for (int i = 0; i < pagePlayer.querySelectorAll("//div[@class='FFsZUeKiSMK9khw9tZrW1']/ancestor::div[@class='_3UmDZyX05ClTVRp6p2xAZj']").size(); i++) {
            titleCollectionAll = pagePlayer.querySelectorAll("//h3[@data-test='PackageListWrapperName']//a");
            pagePlayer.evaluate("t => t.innerText='Название подборки'", titleCollectionAll.get(i));
            counterCollectionAll = pagePlayer.querySelectorAll("//a[@data-test='PackageListWrapperMoreText']");
            pagePlayer.evaluate("c => c.innerText='Все 100'", counterCollectionAll.get(i));
        }
        List<ElementHandle> posterPackageAll;
        List<ElementHandle> titlePackageAll;
        List<ElementHandle> descriptionTextPackage1All;
        List<ElementHandle> ageAll;
        for (int i = 0; i < pagePlayer.querySelectorAll("//div[@class='_3H6SpMZcck2BFXiKBB5gtC _3l_eEMTBvsXXhIcEIbq6Zh']").size(); i++) {
            posterPackageAll = pagePlayer.querySelectorAll("//div[@class='_3H6SpMZcck2BFXiKBB5gtC _3l_eEMTBvsXXhIcEIbq6Zh']");
            pagePlayer.evaluate("p => p.setAttribute('style', 'background-size: cover; " +
                    "background-color: rgb(238, 238, 238); " +
                    "background-image: url(https://static-sesure.cdn.megafon.tv/images/Mixed/35/ee/4cb8df5e72d867ab46ba2da8169c56159b71/tile__web-wp.webp);')", posterPackageAll.get(i));
            titlePackageAll = pagePlayer.querySelectorAll("//h3[@data-test='PackageDescriptionTitle']");
            pagePlayer.evaluate("t => t.innerText='Название фильма'", titlePackageAll.get(i));
            descriptionTextPackage1All = pagePlayer.querySelectorAll("//span[@class='_1VOD2HVjO24JlwN9I3tRYd']//span");
            pagePlayer.evaluate("d => d.textContent='2021, Жанр'", descriptionTextPackage1All.get(i));
            ageAll = pagePlayer.querySelectorAll("//div[contains(@class,'_3RTKiE8VDgo764HGa4WvpJ _3uK4RWVSuUFLQ2ZmeFzsQi')]");
            pagePlayer.evaluate("a => a.innerText='18+'", ageAll.get(i));
        }

        List<ElementHandle> stickerAll = pagePlayer.querySelectorAll("//div[@class='_33eseRmm5G3s8cqfYtR0dR _20sDPzuxbeD_zlQAuQfNyy']");
        ...
        for (int i = 0; i < pagePlayer.querySelectorAll("//div[@class='_3H6SpMZcck2BFXiKBB5gtC _2wlpAXpsfCG6-Gto_H43-B']").size(); i++) {
            posterCollection2All = pagePlayer.querySelectorAll("//div[@class='_3H6SpMZcck2BFXiKBB5gtC _2wlpAXpsfCG6-Gto_H43-B']");
            pagePlayer.evaluate("pC => pC.setAttribute('style', 'background-image: url(\"https://static-sesure.cdn.megafon.tv/images/Collection/44/c9/3b579a939eed7a7301285886d33dc68bcdb8/tile_collection_group__web-wp.webp\");')", posterCollection2All.get(i));
            titleCollection2All = pagePlayer.querySelectorAll("//h3[@data-test='CollectionName']");
            pagePlayer.evaluate("t => t.innerText='Название подборки (18+)'", titleCollection2All.get(i));
        }
        // подготовка баннеров:
        List<ElementHandle> dotBefore = pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']");
        if (dotBefore.size() > 22) {
            while (pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size() != 22) {
                pagePlayer.evaluate("dU => dU.remove();", page.querySelector("(//button[@data-test='CarouselDotButton'])[last()]"));
                System.out.println(pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size());
            }
        }
        if (dotBefore.size() < 22) {
            while (pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size() != 22) {
                pagePlayer.evaluate("newDot => newDot.appendChild(document.getElementsByClassName('_3Na2_ekd58nusdf4LI3vt0')[0].cloneNode(true));", pagePlayer.querySelector("//div[@class='_2-F_qEwyH9P_zWeUdZcMcd _77CQGroIvaqgGukdVHQ7X']"));
                System.out.println(pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size());
            }
        }
        pagePlayer.click("(//div[@class='_2-F_qEwyH9P_zWeUdZcMcd _77CQGroIvaqgGukdVHQ7X']//button[@data-test='CarouselDotButton'])[2]");
        ElementHandle changeTransform = pagePlayer.querySelector("//div[@class='_1kVeVZ_VGnmjl8qGdsFyY9']");
        pagePlayer.evaluate("ch => ch.setAttribute('style', 'transform: translateX(0px); transition-duration: 0ms;')", changeTransform);
        pagePlayer.click("(//div[@class='_2-F_qEwyH9P_zWeUdZcMcd _77CQGroIvaqgGukdVHQ7X']//button[@data-test='CarouselDotButton'])[1]");
        ElementHandle titleBanner = pagePlayer.querySelector("(//div[@data-test='SlideTitle'])[2]");
        pagePlayer.evaluate("t => t.innerText='Название фильма'", titleBanner);
        ElementHandle stiker = pagePlayer.querySelector("(//div[@class='kjFUbLahFxqq2AjHY8j2R'])[2]");
        pagePlayer.evaluate("s => s.innerText='Автотест'", stiker);
        ElementHandle colorStiker = pagePlayer.querySelector("(//div[@class='_33eseRmm5G3s8cqfYtR0dR _1AqbFwoLFYTG_y6SmXKfYb'])[2]");
        pagePlayer.evaluate("c => c.setAttribute('style', 'background-color: rgb(247, 121, 28); color: rgb(255, 255, 255); box-shadow: none;')", colorStiker);
        ElementHandle yearAndGenre = pagePlayer.querySelector("(//div[@data-test='SlideDescription'])[2]");
        pagePlayer.evaluate("yG => yG.innerText='2021, Жанр'", yearAndGenre);
        ElementHandle age = pagePlayer.querySelector("(//div[contains(@class,'_2lExk7vBwXdikRmrmW8iJ_ _3ee886k9ILen72-SjCqCR6')])[2]");
        pagePlayer.evaluate("a => a.innerText='18+'", age);
        ElementHandle poster = pagePlayer.querySelector("(//div[@class='_3H6SpMZcck2BFXiKBB5gtC dc5b4FeyE5AXbLbUa66RW'])[2]");
        pagePlayer.evaluate("p => p.setAttribute('style', 'background-image: url(https://static-sesure.cdn.megafon.tv/images/Film/ba/cb/c68eb9f98803b40eb41f8b6e984f17953846/poster__web-wp.webp);')", poster);
        System.out.println(pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size());

        // делаем скриншот полной страницы "FilmsPageScrollForUnauthorized":
        vrt.track(
                "FilmsPageScrollForUnauthorized",
                Base64.getEncoder().encodeToString(pagePlayer.screenshot(new Page.ScreenshotOptions().setFullPage(true))),
                TestRunOptions.builder()
                        .device("Acer")
                        .os("Win10 Pro")
                        .browser("Chrome")
    ...
    public void checkImageFilmsPageScrollForAuthorized() throws IOException, InterruptedException {
        ...
        pagePlayer.evaluate("uL => uL.innerText='+79260010101'", userLogin);
        List<ElementHandle> titleCollectionAll;
        List<ElementHandle> counterCollectionAll;
        for (int i = 0; i < pagePlayer.querySelectorAll("//div[@class='FFsZUeKiSMK9khw9tZrW1']/ancestor::div[@class='_3UmDZyX05ClTVRp6p2xAZj']").size(); i++) {
            titleCollectionAll = pagePlayer.querySelectorAll("//h3[@data-test='PackageListWrapperName']//a");
        ...
        List<ElementHandle> stickerAll;
        for (int i = 0; i < pagePlayer.querySelectorAll("//div[@class='_33eseRmm5G3s8cqfYtR0dR _20sDPzuxbeD_zlQAuQfNyy']").size(); i++) {
            stickerAll = pagePlayer.querySelectorAll("//div[@class='_33eseRmm5G3s8cqfYtR0dR _20sDPzuxbeD_zlQAuQfNyy']");
            pagePlayer.evaluate("s => s.remove();", stickerAll.get(i));
        }
        // подготовка блоков подборок без заголовка:
        List<ElementHandle> posterCollection2All;
        List<ElementHandle> titleCollection2All;
        ...
        if (dotBefore.size() > 22) {
            while (pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size() != 22) {
                pagePlayer.evaluate("dU => dU.remove();", pagePlayer.querySelector("(//button[@data-test='CarouselDotButton'])[last()]"));
                System.out.println(pagePlayer.querySelectorAll("//button[@data-test='CarouselDotButton']").size());
            }
        }
        if (dotBefore.size() < 22) {
