...
class TestBasePlaywright extends BasePagePlaywright{
    ...
    void launchBrowser() {
        ...
        browserIncognitoModeHeadless = playwright.chromium().launch(new BrowserType.LaunchOptions().setChannel("chrome").setProxy(new Proxy("http://proxy.megalabs.ru:8808")).setHeadless(true).setArgs(Arrays.asList("--disable-dev-shm-usage")).setArgs(Arrays.asList("--whitelisted-ips")));
